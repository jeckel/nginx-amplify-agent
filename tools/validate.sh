...
_usage () {
    echo "Usage: $SCRIPT [--help] filename" >&2
    exit 1
}
...
_help () {
    nginx -v
    echo "Usage: $SCRIPT [--help] filename"
    echo
    echo 'Options:'
...
do_command () {
    local OUTPUT_FILE="/tmp/$SCRIPT.out"
    local RETURN_CODE=0
...
FILEPATH=`python -c "import os; print os.path.abspath('$1')"` \
&& do_command "analyzing syntax of $FILEPATH" "nginx -c $FILEPATH -t" \
