...
from amplify.agent.objects.plus.api import NginxApiStreamServerZoneObject
...
class ApiStreamServerZoneCollectorTestCase(BaseTestCase):
    def setup_method(self, method):
        super(ApiStreamServerZoneCollectorTestCase, self).setup_method(method)
        context.plus_cache = None
        context._setup_plus_cache()

    def test_gather_data(self):
        stream = NginxApiStreamServerZoneObject(local_name='some_stream', parent_local_id='nginx123', root_uuid='root123')
        stream.api_internal_url_cache = 'test_api'
