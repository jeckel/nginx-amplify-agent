...
from amplify.agent.objects.plus.api import NginxApiHttpServerZoneObject
...
class ApiHttpServerZoneCollectorTestCase(BaseTestCase):
    def setup_method(self, method):
        super(ApiHttpServerZoneCollectorTestCase, self).setup_method(method)
        context.plus_cache = None
        context._setup_plus_cache()

    def test_gather_data(self):
        status_zone = NginxApiHttpServerZoneObject(local_name='hg.nginx.org', parent_local_id='nginx123', root_uuid='root123')
        status_zone.api_internal_url_cache = 'test_api'
