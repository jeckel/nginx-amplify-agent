...
from amplify.agent.objects.plus.object import NginxStreamUpstreamObject
...
class StreamUpstreamCollectorTestCase(BaseTestCase):
    def setup_method(self, method):
        super(StreamUpstreamCollectorTestCase, self).setup_method(method)
        context.plus_cache = None
        context._setup_plus_cache()

    def test_gather_data(self):
        stream_upstream = NginxStreamUpstreamObject(
            local_name='backend',
            parent_local_id='nginx123',
            root_uuid='root123'
        )
        stream_upstream.plus_status_internal_url_cache = 'test_status'
